...
- 돌아온 값에 `problems` 가 있으면 그림 한 장이 들어가지 않은 것이다. 그 내용을 작가에게 전한다.

### 그림 올리기

그림은 계획서 폴더의 `images/` 에 둔다. 한 장씩 base64(글자로 바꾼 파일)로 바꿔 올리고, `plan.md` 에
그림 줄을 쓴 다음 계획서를 올린다. **그림을 먼저 올리고 계획서를 올린다.** 순서가 바뀌면 문서에
「그림을 찾지 못했습니다」가 들어간다. 같은 이름으로 다시 올리면 문서의 그림도 바뀐다.

```bash
# macOS · Linux — images/sketch1.jpg 올리기
base64 < images/sketch1.jpg | curl -sL --data-binary @- -H "Content-Type: text/plain" "$(cat .plan-sync)&op=image&name=sketch1.jpg"
# 올린 그림 목록
curl -sL "$(cat .plan-sync)&op=images"
```

```powershell
# Windows (PowerShell)
[Convert]::ToBase64String([IO.File]::ReadAllBytes("$PWD\images\sketch1.jpg")) | Out-File -Encoding ascii img.b64
curl.exe -sL --data-binary "@img.b64" -H "Content-Type: text/plain" "$u&op=image&name=sketch1.jpg"
Remove-Item img.b64
curl.exe -sL "$u&op=images"
```

- 올릴 수 있는 것은 **jpg, png, gif** 이고 한 장에 15MB 까지다. 아이폰 사진(heic)이나 큰 사진은 먼저 바꾼다.
  macOS 는 `sips -s format jpeg -Z 2000 원본.heic --out images/사진.jpg` 로 jpg 로 바꾸면서 긴 변을 2000px 로 줄인다.
  Windows 는 사진 앱에서 「크기 조정」이나 다른 이름으로 jpg 저장을 하라고 알려 준다.
- 파일 이름은 영문·숫자·하이픈으로 바꿔 둔다(예: `sketch1.jpg`, `space-plan.png`). 한글·띄어쓰기는 피한다.
- `plan.md` 에는 그림 하나를 한 줄에 쓴다. 대괄호 안이 그림 아래에 붙는 설명이다.

```markdown
![처음 떠올린 스케치, 9월 22일](images/sketch1.jpg)
![Cutting Kim 전시 장면 (배준형, 2025). 출처: https://doi.org/10.1145/3757369.3767596](images/ref-cuttingkim.jpg)
```

- 인터넷에 있는 그림은 주소를 그대로 쓸 수 있다: `![설명](https://…/그림.jpg)`. 그림 파일 주소여야 하고
  웹페이지 주소는 안 된다. 잘 안 되면 작가에게 그림을 저장해 `images/` 에 넣어 달라고 한다.
- 링크는 `[글](주소)` 로 쓰거나 주소를 그대로 적는다. 문서에서 눌리는 링크가 된다.
...
2. 전용 주소를 `.plan-sync` 파일에 한 줄로 저장한다. 그림을 둘 `images/` 폴더도 만든다.
...
Tekneh 가 연결되어 있으면 한다. 없으면 이 단계를 건너뛰고 9번 칸은 작가가 아는 작품으로 채운다.
...
4. 작가에게 끌리는 것이 있는지 묻는다. 있으면 9번 칸에 적는다. 링크는 브라우저로 열라고 안내하고,
...
`plan.md` 의 1번부터 13번까지 차례로 간다. 작가가 원하면 순서를 바꾸거나 건너뛴다.
칸이 열세 개라 한 번에 끝내지 않아도 된다고 처음에 알린다. 1·4·6·8·11번이 채워지면 튜터링에서 도울 곳이 보인다.

한 칸마다:

1. 칸의 안내 문장을 보여 주고 질문 **하나**를 한다. 칸 안에 줄이 여럿이면(4·6·8·13번) 두세 줄씩 묶어
   묻고, 표는 한 줄씩 묻는다. 3단계에서 받은 `absent_to_questions` 에 그 칸에 맞는 질문이 있으면 그것을
   쓴다(6번 칸은 sensing·computation·output·software·realtime, 8번은 venue_requirements,
   10번은 process, 13번의 함께 만드는 사람은 collaboration).
2. 작가의 대답을 그 칸에 옮긴다. 안내 문장은 지우고 대답으로 바꾼다. 목록 줄은 「이름:」 부분을 남기고
   뒤만 채운다. 표는 칸 모양(열 수)을 바꾸지 않고, 줄이 모자라면 같은 모양의 줄을 더한다.
3. 진행 상황 표의 그 칸을 고친다: `✅ 채움` / `✏️ 쓰는 중` / `❔ 모름`.
4. 「마지막으로 올린 때」를 고치고 **올린다.**
5. 「다음 칸으로 갈까요?」 하고 기다린다.

대화 중에 작가가 어떤 방향을 접으면 「10번 칸에 한 줄 남겨 둘까요?」 하고 묻는다.
대답이 길어지면 그 칸 안에서 목록으로 나눠 적되, 작가가 하지 않은 말을 더하지 않는다.

칸마다 조심할 것:

- **2번 작가 노트**: 작가가 말한 문장을 옮긴다. 멋진 글로 다시 쓰지 않는다. 짧아도 된다.
- **5번 그림과 스케치**: 작가에게 스케치 사진, 공간 배치를 그린 것, 분위기가 비슷한 사진, 지금까지 만든 것의
  캡처를 `images/` 폴더에 끌어다 놓아 달라고 한다(Cursor 왼쪽 파일 목록에 끌어다 놓으면 된다).
  한 장씩 무엇인지 묻고 그 말을 설명으로 쓴다. 다른 사람의 작품이면 작가 이름과 출처를 꼭 묻는다.
  그림을 만들어 내지 않는다(이미지 생성 금지). 그림이 없으면 「아직 넣은 그림 없음」을 그대로 둔다.
- **6번 기술 구성**: 제품명을 모르면 「모름」, 가지고 있는지는 「있음 / 빌림 / 사야 함」으로 적는다.
  흐름 줄은 작가가 말한 대로 괄호를 채운다.
- **7번 가장 불확실한 것**: 작가가 걱정하는 것을 먼저 묻는다. 네가 걱정거리를 지어내지 않는다.
  작은 실험은 작가와 같이 정하되, 날짜는 11번 칸의 마일스톤보다 앞으로 잡자고 권한다.
- **9번 참고한 작품과 자료**: 3단계의 선례와 작가가 아는 작품을 여기에 모은다. 링크를 붙이고, 그림은
  5번처럼 넣는다. 가져오고 싶은 점과 다른 점은 작가에게 묻는다.
- **11번 일정**: 「프로그램 목표」 열은 고치지 않는다. 「내 목표」 열만 채운다.
- **12번 재료와 비용**: 따로 주어지는 제작비가 없다는 것을 알고 있는 칸이다. 지원이나 구입을 약속하지 않고,
  재단 공통 기자재(프로젝터·모니터·스피커)로 되는지는 기획자에게 물어보라고 한다.
...
> 제출 마감은 9월 30일(수)입니다. 따로 보낼 것은 없고, 그때 구글 문서에 올라가 있는 내용을 봅니다.
>
