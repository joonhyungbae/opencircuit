# React — Closed Circuit 스터디 슬라이드

Closed Circuit Busan 발표를 만드는 시작점입니다.
`src/deck.json` 만 고치면 슬라이드가 바뀝니다. 코드는 건드리지 않아도 됩니다.

사이트에 올라간 1회차 예 → [opencircuit.club/study/cutting-kim](https://opencircuit.club/study/cutting-kim)
(1회차는 여덟 질문으로 했습니다. 이 템플릿은 그것을 열한 칸으로 늘린 것입니다.)

## 먼저 보기

고치기 전에 어떤 슬라이드인지만 보려면, 저장소 루트에서:

```bash
./web.sh          # macOS · Linux
.\web.ps1         # Windows (PowerShell)
```

`http://127.0.0.1:5173` 이 열립니다. 내 발표 폴더를 띄우려면 뒤에 폴더를 적습니다.
예) `./web.sh ~/Documents/OpenCircuit/작품이름-study`

### 다른 컴퓨터에서 볼 때 (SSH · Tailscale)

서버에 SSH 로 붙어 있으면 그 안에는 화면이 없어서 브라우저가 뜨지 않습니다.
**Tailscale 이 떠 있으면 `./web.sh` 가 알아서 tailnet 주소로 엽니다.** 따로 옵션을 붙일 필요가 없습니다.

```bash
./web.sh
[정보] 화면이 없는 세션이라 Tailscale 주소로 엽니다. (127.0.0.1 만 쓰려면 --local)
[성공] 스터디 슬라이드: http://100.x.x.x:5173
[정보] MagicDNS 로도 됩니다: http://<이름>.ts.net:5173
```

**내 노트북·폰 브라우저**에서 그 주소를 엽니다. 보는 기기도 같은 tailnet 에 켜져 있어야 합니다
(`tailscale status` 로 확인).

강제로 켜려면 `--tailscale`, 저절로 켜지는 것을 막으려면 `--local` 입니다.

**Tailscale 이 없으면** 그 사실을 알려 주고 `127.0.0.1` 로 엽니다. 중단하지 않습니다.
이때는 SSH 포트 포워딩 명령을 화면에 찍어 주니 그대로 쓰면 됩니다.

```
[정보] 이 컴퓨터에 Tailscale 이 없습니다.
[정보] 화면이 없는 세션이라 127.0.0.1 로 엽니다. 이 주소는 이 컴퓨터 안에서만 보입니다.
[정보] 내 노트북에서 보려면 그쪽 터미널에서 포트를 넘기세요:
[정보]   ssh -L 5173:127.0.0.1:5173 <사용자>@<이 서버 주소>
```

`0.0.0.0` 이 아니라 tailnet 주소에만 묶으므로, 같은 공유기를 쓰는 다른 사람에게는 보이지 않습니다.



## 쓰는 법

1. 저장소 루트에서 내 발표 폴더를 만듭니다. 손으로 복사하지 않습니다.

   ```bash
   ./web.sh --new 작품이름          # macOS · Linux
   .\web.ps1 -New 작품이름          # Windows
   ```

   문서 폴더에 `OpenCircuit/작품이름-study/` 가 생기고, 절차서와 `npm install` 까지 딸려 옵니다.

2. 직접 하려면 이 `baseline` 폴더를 그 자리에 복사한 뒤 아래를 실행합니다.

**Windows** — PowerShell:

```powershell
Set-ExecutionPolicy -Scope Process Bypass -Force
npm install
npm run dev
```

**macOS · Linux**:

```bash
npm install
npm run dev
```

브라우저가 `http://127.0.0.1:5173` 을 엽니다. 그만할 때는 그 창에서 Ctrl+C.

## 여기를 바꾸세요

- **내용** → `src/deck.json`
- **그림** → `public/` 에 두고, 주소는 `/파일이름.jpg` 처럼 적습니다. 원본을 줄이지 마세요.
- **유튜브** → `{ "kind": "youtube", "src": "https://youtu.be/…" }`

칸은 열한 가지입니다. 페이퍼에 없는 칸은 비워 두고, 다음에 알아볼 과제로 남깁니다.

덱은 다섯 묶음입니다.

| 묶음 | 무엇 |
|---|---|
| 시작 | 표지, 이 페이퍼를 고른 이유 |
| 작품 | 무엇인가 — 한 문장, 영상·사진 |
| 열한 칸 | ① ~ ⑨. 관객에서 시작해 안으로 들어갔다가 작품의 질문으로 나옵니다 |
| 만든 과정 | 시간표 · 계보 · ⑪ 역할 · ⑩ 버린 시도 |
| 토의 | 같이 이야기할 것 셋. 디스커션 10분을 여는 자리 |

⑩ 과 ⑪ 은 「열한 칸」이 아니라 「만든 과정」에 있습니다. 번호 순서가 아니라
**제작 서사**로 묶는 편이 발표에 낫기 때문입니다. 이 둘은 논문에 없는 경우가
흔합니다 — 그러면 비우고 넘어가면 됩니다. 시간표·계보도 논문이 안 적었으면 지웁니다.

| # | 칸 | 무엇을 적나 |
|---|---|---|
| 1 | 관객은 무엇을 하나 | 몸, 소리, 시간, 또는 아무것도 하지 않는지 |
| 2 | 어떤 신호를 받나 | 카메라, 센서, 마이크. 기종까지 |
| 3 | 계산은 어디에서 이루어지나 | 노트북인지, 서버인지, 마이크로컨트롤러인지 |
| 4 | 무엇이 실시간이고 무엇이 미리 만든 것인가 | 지금 바뀌는 것과 미리 만들어 둔 것 |
| 5 | 어떤 소프트웨어로 만들었나 | 게임엔진, TouchDesigner, 직접 짠 코드 |
| 6 | 무엇이 출력되나 | 화면, 프로젝션, 스피커. 개수와 크기 |
| 7 | 전시장에서는 무엇이 필요했나 | 전력, 암전, 천장고, 지킴이 |
| 8 | 공개했나 | 코드·도면·데이터셋 URL. 흉내가 아닙니다 |
| 9 | 작품이 던지는 질문은 무엇인가 | 한 문장. 논문이 자기 계보로 든 이름만 |
| 10 | 버린 시도·실패·반복은 무엇인가 | 버전, 실패한 시도, 전시를 돌며 바뀐 것 |
| 11 | 누가 무엇을 했나 | 역할. 저자 목록만으로는 부족합니다 |

## 조작

| 키 | 하는 일 |
|---|---|
| ← → · 스페이스 · PageUp/Down | 장 넘기기 |
| 아래 막대의 버튼 | 같은 일 |
| F | 전체 화면 |
| 주소 `#7` | 7번째 장으로 바로 열기 |

화면을 클릭해서는 넘어가지 않습니다. 키보드나 클리커를 씁니다.

## 블록

`deck.json` 의 `blocks` 에 넣는 종류입니다. 예는 템플릿 덱에 있습니다.

| kind | 무엇 | 옵션 |
|---|---|---|
| `lead` | 테두리 친 한 문단 | `text` |
| `big` | 큰 글자 한 줄 | `text`, `sub` |
| `bullets` | 점 목록 | `items` |
| `cards` | 제목 있는 카드 | `items[{title, lines 또는 text}]`, `cols` 1·2·3 (기본 2) |
| `steps` | 단계 카드 | `items[{no, label, desc, color}]`, `stack: true` 면 세로로 쌓음(시간표) |
| `table` | 표 | `head`, `rows` |
| `qa` | 질문·답 | `items[{q, a}]` |
| `note` | 옅은 배경 메모 | `text`, `tone: "warn"` 이면 노란 배경 |
| `figure` | 그림 한 장 | `src`, `caption`, `size: "lg"` 면 칸을 가득 채움 |
| `youtube` | 유튜브 | `src`(watch·youtu.be·shorts 주소), `caption` |
| `row` | 왼쪽·오른쪽 나눔 | `left`, `right` 에 블록 배열 |

글 안에서 `**굵게**` 와 주소 자동 링크가 됩니다.

## 이 칸이 어디서 왔나 — `source`

각 장에 `source` 를 달면 제목 옆에 배지가 뜹니다. **논문이 적은 것과 발표자가 채운 것을
구분하는 자리**입니다. 안 달면 배지가 안 뜹니다.

| `source` | 배지 | 언제 |
|---|---|---|
| `paper` | 논문에 적힘 | 논문이 적었다 |
| `inferred` | 논문에서 유추 | 논문에서 유추했다. 유추임을 문장에도 밝힙니다 |
| `absent` | **논문에 적히지 않음** | 논문이 안 적었다. 노란 배지 |
| `reader` | 발표자가 채움 | 논문 밖에서 알아낸 것 |
| `na` | 해당 없음 | 이 작품에 해당하지 않는다 |

`absent` 는 **「없었다」가 아닙니다.** 논문이 안 적었을 뿐, 있었는지 없었는지는 모릅니다.
「없었다 · 안 했다 · 사용하지 않았다」로 바꿔 적지 마세요. 사실이 아닌 말이 됩니다.

⑧번 장이 이 용례로 들어 있습니다.
표지는 `cover` 입니다. 섹션 색은 `"212 80% 45%"` 처럼 HSL 세 숫자만 적습니다.

## 안 될 때

- **브라우저가 안 열린다.** `./web.sh --doctor` 를 실행하면 이유를 알려 줍니다.
  `DISPLAY` 가 비어 있으면 화면이 없는 세션(SSH·콘솔)이라 어떤 브라우저도 뜰 수 없습니다 — 위의 Tailscale 절을 보세요.
- **Tailscale 주소로 열었는데 「Blocked request」 가 뜬다.** `--tailscale` 없이 띄운 것입니다.
  Vite 가 낯선 주소를 막습니다. `--tailscale` 로 다시 띄우세요.
- **포트가 이미 쓰인다.** `npm run dev -- --port 5174` 로 다른 문을 엽니다.
- **그림이 안 보인다.** 파일이 `public/` 안에 있는지, 주소가 `/` 로 시작하는지 봅니다.
- **글꼴이 다르게 보인다.** Pretendard 와 JetBrains Mono 는 인터넷에서 받아 옵니다. 발표 장소에 인터넷이 없으면 시스템 글꼴로 바뀌어 줄 바꿈이 달라질 수 있으니, 미리 한 번 열어 보세요.
- **JSON 오류.** 쉼표가 빠졌거나 따옴표가 짝이 안 맞습니다. Cursor 에 `deck.json` 을 열고 물어보세요.
