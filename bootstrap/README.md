...
부트스트랩은 `tekneh` 도 함께 등록합니다. SIGGRAPH · SIGGRAPH Asia 아트 페이퍼
2009–2026 작품 논문 229편이 **논문이 무엇을 적었고 무엇을 안 적었는지** 기준으로
정리되어 있습니다. 스터디 발표 슬라이드를 만들 때 씁니다 — `prompts/04-study-slides.md`.

- 파이썬 패키지라 `uv` 가 필요합니다. 부트스트랩이 알아서 깝니다
  (`~/.local/bin`, 관리자 암호 안 물어봅니다)
- **선택 사항입니다.** 설치에 실패해도 부트스트랩은 계속 진행하고,
  나머지 도구와 슬라이드 템플릿은 그대로 동작합니다
- 처음 부를 때 패키지를 받느라 1~2분 걸릴 수 있습니다.
  부트스트랩이 미리 받아 두므로 보통은 기다릴 일이 없습니다
